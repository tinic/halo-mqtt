    ...
    async def _send_packet(self, packet: bytes) -> bool:
        ...
        for _ in range(3):
            try:
                ...
                self.mesh_connection = None
                logging.getLogger(__name__).exception("Caught exception connecting to device")
...
async def load():
    try:
        locations = await halohome.list_devices(
            "tinicuro@gmail.com", "R2d23028R2d23028", product_ids = [162]
        )
    except (HaloHomeError, OSError, aiohttp.ClientError) as exception:
        print("What!!!\n")
        return

    connection = Connection(locations, timeout = 20)

    for device in connection.devices:
        print(device.location.location_id)
        print(device.location.mesh_connection)
        print(device.device_id)
        print(device.device_name)
        print(device.pid)
        print(device.mac_address)
        print(device.location.timeout)
        device.location.timeout = 20
